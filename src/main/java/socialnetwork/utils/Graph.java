...
public class Graph {
     ...
     */
    private void DFSUtil(int v, boolean[] visited,ArrayList<Integer> componentNode) {
        visited[v] = true;
        componentNode.add(v);
        for (int x : adjListArray[v]) {
            if(!visited[x]) DFSUtil(x,visited,componentNode);
        }
    }
    ...
    public int connectedComponents() {
        ...
        for(int v = 1; v < V; ++v) {
            ...
            if (!visited[v]) {
                DFSUtil(v, visited,componentNode);
            }
